...
from probe_generator import exon_coordinate
...
class AbstractSequenceRangeTestCase(unittest.TestCase):
    """Provide setUp function for `exon_coordinate.sequence_range` tests.
...
class TestSequenceRangePositional(AbstractSequenceRangeTestCase):
    """Test cases for the exon_coordinate.sequence_range method using the
    `positional` strategy.
    ...
    def test_sequence_range_returns_inclusive_ranges_and_rev_comp_flags(self):
        ...
        """
        range_spec = exon_coordinate.sequence_range(
                self.probe_specification,
                self.feature_1,
                self.feature_2)
        self.assertEqual(
    ...
    def test_raises_InterfaceError_when_specification_missing_feature(self):
        with self.assertRaises(exon_coordinate.InterfaceError):
            del self.probe_specification['feature1']
            exon_coordinate.sequence_range(
                    self.probe_specification,
                    self.feature_1,
                    self.feature_2)
    ...
    def test_raises_InterfaceError_when_row_missing_strand(self):
        with self.assertRaises(exon_coordinate.InterfaceError):
            ...
            print(exon_coordinate.sequence_range(
                    self.probe_specification,
                    self.feature_1,
                    self.feature_2))
    ...
    def test_raises_NoFeatureError_when_exon_out_of_range(self):
        ...
        with self.assertRaisesRegex(exon_coordinate.NoFeatureError, message):
...
class TestSequenceRangesEndwise(AbstractSequenceRangeTestCase):
    ...
    `exon_coordinate.sequence_range` using the positional strategy.
...
class TestSequenceRangeReadThrough(TestSequenceRangePositional):
    """Test cases for the read-through strategy for exon_coordinate.sequence_range
    ...
    def chromosomes_swapped(self):
        ...
        returned by exon_coordinate.sequence_range corresponds to
        self.feature2['chrom'] and vice versa.

        """
        coords = exon_coordinate.sequence_range(
                    self.probe_specification,
                    self.feature_1,
                    self.feature_2)
    ...
    def assert_warning_raised(self, warning):
        ...
        exon_coordinate.sequence_range was called with the standard parameters 
        """
        exon_coordinate.sequence_range(
                self.probe_specification,
                self.feature_1,
    ...
    def test_warning_raised_when_side_1_is_start(self):
        ...
        self.assert_warning_raised(exon_coordinate.WARNING_MESSAGE)
...
class TestExonsIntegrationPositional(TestSequenceRangePositional):
    """Integration tests for `exon_coordinate.sequence_range` and `annotation.exons`
